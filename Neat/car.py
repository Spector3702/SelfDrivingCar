...
class Car:
    ...
    def getInputs(self, world, road,CAR_DBG):         #如果 DBG=True，則 win 用於繪製傳感器
        sensors = []
        for k in range(8):
            sensors.append(SENSOR_DISTANCE) # SENSOR_DISTANCE = 200
        sensorsEquations = getSensorEquations(self, world, CAR_DBG)
        ...
        if CAR_DBG:
            for k,s in enumerate(sensors):
                ...
                if s < SENSOR_DISTANCE:
                    py.draw.circle(world.win,BRIGHT_RED, world.getScreenCoords(self.x+dx, self.y+dy), 7)
...
def getSensorEquations(self, world,CAR_DBG):       #restituisce le equazioni delle rette (in variabile y) della macchina in ordine [verticale, diagonale crescente, orizzontale, diagonale decrescente]
    eq = []
    for i in range(4):
        ...
        if CAR_DBG:             #disegna linee dei sensori
            py.draw.lines(world.win, DARK_GREEN_PALE, False, [world.getScreenCoords(self.x+dx, self.y+dy), world.getScreenCoords(self.x-dx, self.y-dy)], 2)
